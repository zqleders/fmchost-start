...
def send_tg_notification(message: str, image_path: str = None):
    """同步调用 Telegram 消息与截图发送（带流控自动等待重试机制）"""
    ...
    async def _send():
        ...
        for attempt in range(2):
            try:
                ...
                print(f"[WARN] ⚠️ 触发 Telegram 频率限制，等待 {wait_time} 秒后重试...")
                await asyncio.sleep(wait_time)
            except TelegramError as e:
                print(f"[ERROR] Telegram 发送文本失败: {e}")
                break
        ...
        if image_path and os.path.exists(image_path):
            for attempt in range(2):
                try:
                    ...
                    wait_time = e.retry_after + 2
                    print(f"[WARN] ⚠️ 发送图片触发频率限制，等待 {wait_time} 秒后重试...")
                    await asyncio.sleep(wait_time)
                except TelegramError as e:
                    print(f"[ERROR] Telegram 发送图片失败: {e}")
                    break

    try:
        asyncio.run(_send())
    except Exception as e:
        print(f"[ERROR] 发送 Telegram 消息捕获到未预期异常: {e}")
...
def run():
    ...
    # 启动 SeleniumBase UC 模式（关闭 headless，结合 Actions 的 Xvfb 进行物理点击）
